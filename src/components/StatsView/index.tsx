...
const StatsView: React.FC<StatsViewProps> = ({ data }) => {
  ...
  const regionStats = data.regions.map(region => ({
    ...
    area: region.area,
    density: Math.round(region.population / region.area),
    universities: data.universities.filter(uni => uni.region === region.name).length
  }));
  ...
  const universityTypes = [
    {
      name: 'Publiques',
      value: data.universities.filter(uni => uni.type === 'Public' || !uni.type).length,
      color: '#3B82F6'
    },
    {
      name: 'Privées',
  ...
  const populationDistribution = [
    { range: '< 1M', count: data.regions.filter(r => r.population < 1000000).length, color: '#EF4444' },
    { range: '1M - 2M', count: data.regions.filter(r => r.population >= 1000000 && r.population < 2000000).length, color: '#F59E0B' },
    { range: '2M - 3M', count: data.regions.filter(r => r.population >= 2000000 && r.population < 3000000).length, color: '#10B981' },
    { range: '3M+', count: data.regions.filter(r => r.population >= 3000000).length, color: '#3B82F6' }
  ];
  ...
  const radarData = regionData.slice(0, 6).map(region => ({
    ...
    universities: universityByRegion.find(u => u.name === region.fullName)?.universities || 0
  }));
  ...
  const renderChart = () => {
    switch (activeChart) {
          ...
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={regionData}>
              ...
              <Tooltip 
                ...
                labelFormatter={(label) => regionData.find(r => r.name === label)?.fullName || label}
                contentStyle={{ 
        ...
        return (
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={universityByRegion}>
              ...
              />
              <Bar yAxisId="left" dataKey="universities" fill="#8B5CF6" radius={[4, 4, 0, 0]} />
              <Line yAxisId="right" type="monotone" dataKey="students" stroke="#F59E0B" strokeWidth={3} />
            </ComposedChart>
          </ResponsiveContainer>
        );
        ...
        return (
          <ResponsiveContainer width="100%" height={400}>
            <AreaChart data={regionStats}>
              ...
              />
              <Area type="monotone" dataKey="density" stackId="1" stroke="#3B82F6" fill="#3B82F6" fillOpacity={0.6} />
              <Area type="monotone" dataKey="universities" stackId="1" stroke="#10B981" fill="#10B981" fillOpacity={0.6} />
            </AreaChart>
          </ResponsiveContainer>
        );
      ...
      case "trends":
        return (
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={universityData.sort((a, b) => a.founded - b.founded)}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-lg text-gray-600">
          Analyses détaillées et visualisations interactives des données du Cameroun
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={Math.round(data.overview.totalPopulation / data.overview.totalArea) + " hab/km²"}
          icon={MapPin}
        ...
        <StatCard
          ...
          value={data.universities.reduce((sum, uni) => sum + (uni.students || 0), 0).toLocaleString()}
          icon={GraduationCap}
        ...
        <StatCard
          ...
          value={Math.round(data.universities.reduce((sum, uni) => sum + (new Date().getFullYear() - uni.founded), 0) / data.universities.length) + " ans"}
          icon={Activity}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <motion.div
          ...
          className="bg-white rounded-xl shadow-lg p-6 border border-gray-100"
        >
          <h3 className="text-xl font-bold text-gray-900 mb-4">Top 5 Régions par Population</h3>
          <div className="space-y-3">
            {regionData.slice(0, 5).map((region, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <span className="text-gray-600">{region.population.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-white rounded-xl shadow-lg p-6 border border-gray-100"
        >
          <h3 className="text-xl font-bold text-gray-900 mb-4">Universités par Région</h3>
          <ResponsiveContainer width="100%" height={200}>
            <ScatterChart data={scatterData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              <XAxis dataKey="x" name="Superficie" unit=" km²" tick={{ fontSize: 12 }} />
              <YAxis dataKey="y" name="Population" tick={{ fontSize: 12 }} />
              <ZAxis dataKey="z" range={[50, 400]} name="Densité" unit=" hab/km²" />
              <Tooltip 
                cursor={{ strokeDasharray: '3 3' }}
                formatter={(value, name) => [
                  name === 'x' ? value.toLocaleString() + ' km²' :
                  name === 'y' ? value.toLocaleString() :
                  value + ' hab/km²',
                  name === 'x' ? 'Superficie' :
                  name === 'y' ? 'Population' : 'Densité'
                ]}
                contentStyle={{ 
                  backgroundColor: 'white', 
                  border: '1px solid #e5e7eb',
